...
describe('output formatter as used by CLI', () => {
  ...
  it('table() renders tab-separated rows when in a TTY', () => {
    const out = createOutput({ isTTY: true });
    out.table([{ name: 'foo', count: 3 }]);
    const calls = stdoutSpy.mock.calls.map((c) => c[0]).join('');
    expect(calls).toContain('name\tcount');
    expect(calls).toContain('foo\t3');
  });

  it('success() goes to stderr (so stdout stays machine-parseable)', () => {
    const out = createOutput({ isTTY: true });
    out.success('hello');

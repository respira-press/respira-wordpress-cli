...
export const sitesDisconnectFunction: ToolChainFunction<boolean> = {
  name: 'sites.disconnect',
  description: 'remove a site from ~/.respira/sites.json (does not revoke the WP plugin key)',
  domainTags: ['sites', 'write', 'destructive'],
  capability: 'destructive',
  prerequisites: [],
  async execute(input) {
    const { site, baseUrl } = input as { site: string; baseUrl?: string };
    const client = createRespiraClient({ baseUrl, anonymous: true });
    return client.sites.disconnect(site);
  },
};
...
export default class SitesDisconnect extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    try {
      const removed = await this.runThroughCycle(
        sitesDisconnectFunction,
        { site: args.site, baseUrl: flags['base-url'] },
        { toolName: 'sites disconnect', task: { site: args.site } },
      );
      if (!removed) {
